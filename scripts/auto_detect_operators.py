...
def loadModel(process):
  ...
  sys.path.append(os.path.join(EFT2OBS_DIR, MG_DIR, "models", model_name))
...
def makeConfig(process, model, params, args):
  ...
  command = "python3 %s/scripts/make_config.py -p %s -o %s/%s/config.json --pars %s"%(EFT2OBS_DIR, process, CARDS_DIR, process, block_param_nums) + extra_args  
  print(command)
  os.system(command)

def makeReweight(process):
  print(">> Making reweight card")
  command = "python3 %s/scripts/make_reweight_card.py %s/%s/config.json %s/%s/reweight_card.dat"%(EFT2OBS_DIR, CARDS_DIR, process, CARDS_DIR,process)
  os.system(command)
